...
We seek to provide results for all countries in all WHO regions. For technical
reasons (missing data, low incidence, model not successfully fitting to the
data), results may not be available for some countries. Where this happens, the
list of missing countries is indicated in the relevant sections.
...
1. *% of historical peak of case incidence*: this metric uses the highest case
   incidence reported over a weekly time window, excluding the last 90 days of
   data; this metric works in most cases, but there are several caveats:
    * the last peak may have happened in the last 90 days
    * some countries with a slow, continuous build-up of cases and no previous
     waves do not have a 'peak'
    * increases in case incidence may reflect increases in testing/surveillance
