...
vax_data <- vax_data  %>%
  ...
  group_by(iso3) %>% 
  arrange(date) %>% 
  fill(vac_oneplus_dose, .direction = "down") %>% 
  fill(vac_full, .direction = "down") %>% 
  mutate(across(starts_with("vac_"), ~replace_na(0))) %>% 
  ungroup()


final_dat <- final_dat %>%
  left_join(vax_data %>% 
              select(-population, -who_region),
            by = c("report_date" = "date", "iso3" = "iso3"))
...
final_dat <- final_dat %>% 
  ...
  # filter out rows where there is no country
  filter(!is.na(report_country)) %>% 
