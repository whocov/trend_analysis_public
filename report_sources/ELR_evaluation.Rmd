...
ELR <- ELR_raw %>% 
  ...
  mutate(
    ...
    relative_incidence = replace(relative_incidence, relative_incidence > 200, NA),
          
    algorithm_classification = recode(algorithm_classification,
                                              "Mid" = "Medium"),
    dynamics = recode(
...
```{r}
ELR %>% 
  filter(final %in% c("1 - Critical", "2 - Very high")) %>% 
  count(week) %>% 
  ggplot()+
  geom_line(aes(x = week, y = n), size = 1)+
  theme_minimal()+
  labs(title = "Number of Critical/Very high countries",
       subtitle = "(this impacts the workload of Intel team)")
...
ELR %>% 
  count(week, final) %>% 
  ggplot()+
  geom_line(aes(x = week, y = n, color = final), size = 1)+
...
We examine the proportion of countries in a given week that were detected by the ELR process prior to their becoming a hotspot. That is, for the week when a country was *first* classified as either `r alert`, were they listed in the previous two weeks as one of:  `r warning`?  
...
# Trajectories of "Medium" countries  {.tabset}  
...
This plot depicts the trajectories of countries after they are first classified as "Medium" (day 0 on the x-axis). There are two distinct patterns:  
...
If we look deeper and try to understand the epidemiological characteristics of countries classified as "Medium", they tend to be either:  
...
The plot below shows this dynamic, as most Medium countries are either in the upper-left, or the lower-right. While countries in both sectors did increase to Critical/Very High ELR within a few weeks (red), countries in the upper-left with higher incidence and lower growth were more likely to increase.  
