...
<!--   geom_stratum(width = 1/8, aes(fill = `2021-07-19`), color = "grey") + -->
<!--   #geom_label(stat = "stratum", size = 1.5, aes(label = after_stat(stratum))) + -->
...
<!-- Visualization with alternative R package   -->
...
df_high  <- consistency %>% 
  ...
  select(
    country, week, who_region, growth_rate, relative_incidence, net_increases_asmodee, dynamics_lvl, final_lvl, class, first, day_first,
...
mutate(
  first_row = ifelse(row_number() == 1, TRUE, FALSE)) %>% 
     # mutate(first_alert_day = first_alert,
     #        first_alert_day = fill(first_alert_day))
     mutate(final_lag1 = lag(final),
            final_lag2 = lag(final, 2)) %>% 
...
medium_data <- consistency %>% 
  ...
  mutate(post_med_5 = lead(final, 5)) %>% 
    ...
    # Create columns that look ahead 5 weeks
  mutate(pre_med_1 = lag(final, 1)) %>% 
  mutate(pre_med_2 = lag(final, 2)) %>% 
  mutate(pre_med_3 = lag(final, 3)) %>% 
  mutate(pre_med_4 = lag(final, 4)) %>% 
  mutate(pre_med_5 = lag(final, 5)) %>% 
  
  
  # identify countries on downward trajectory
  
    # Via ELR
    mutate(downward_elr = ((pre_med_1 %in% c(alert) | 
                              pre_med_2 %in% c(alert) | 
                              pre_med_3%in% c(alert) | 
                              pre_med_4 %in% c(alert) | 
                              pre_med_5 %in% c(alert)))) %>% 
  ...
  # converts all rows in a country to value at first Medium
  mutate(medium_class = medium_class[first_med == 1]) %>% 
  ...
  select(
    country, week, who_region, growth_rate, relative_incidence, net_increases_asmodee, dynamics_lvl, final_lvl, medium_class, first_med, day_first_med,
    pre_med_5, pre_med_4, pre_med_3, pre_med_2, pre_med_1,
    final,
    post_med_1, post_med_2, post_med_3, post_med_4, post_med_5) %>% 
 
  # Number of weeks to hit Critical / Very High  
  mutate(weeks_to_alarm = case_when(
      (post_med_1 %in% alert) ~ 1,
      (post_med_2 %in% alert) ~ 2,
      (post_med_3 %in% alert) ~ 3,
      (post_med_4 %in% alert) ~ 4, 
      (post_med_5 %in% alert) ~ 5)) %>% 
  
  group_by(country) %>% 
  filter(any(first_med == 1)) %>% 
...
# 
# 
# tabyl(medium_data$medium_class)
# other <- filter(medium_data, medium_class == "Other")
# plot(other$growth_rate, other$relative_incidence)
```
...
medium_data %>% 
  #filter(medium_class == "High, stable") %>% 
  ggplot() +
  geom_line(aes(x = day_first_med, y = final, group = country), color = "grey", alpha = 0.3, size = 1)+
