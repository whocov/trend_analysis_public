...
The final ELR classification by country - globally (difficult to read - see regional-specific alluvial plots in the section below).  
...
```{r message = F, out.height="200%", out.width="200%"}
 ELR %>% 
     select(week, country, who_region, dynamics, context, final) %>% 
     pivot_longer(cols = c(dynamics, context, final), names_to = "metric", values_to = "value") %>% 
     filter(metric == "final") %>% 
...
```{r, warning=F, message=F, out.height="150%", out.width="150%"}
# plot showing current classification
ggplot(data = ELR_long_alluvial,
       aes(x = date, stratum = stratum, alluvium = alluvium,
           y = n, label = stratum)) +
