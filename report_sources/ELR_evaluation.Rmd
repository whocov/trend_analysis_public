...
ELR_all <- ELR_raw %>% 
  ...
  mutate(
    ...
    final = case_when(
      is.na(final) & is.na(context) & dynamics == "Insufficient data (chronic)" ~ "No data",
  ...
  mutate(final_lvl = case_when(
    ...
    final == "5 - Low" ~ 4,
    final == "6 - Minimal" ~ 5,
    final %in% c("Unknown", "Insufficient data", "No data") ~ 6)) %>% 
...
map_data <- who_shp$adm0 %>% 
  ...
  mutate(final = na_if(final, "No data"))
