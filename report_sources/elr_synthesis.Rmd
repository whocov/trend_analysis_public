...
if (params$use_decoupling) {
     x <- x %>% 
          left_join(all_elr_extras %>% 
                         select(iso3, vac_coverage = vaccinations_per_100)) %>% 
          left_join(dc_dat %>% 
                         group_by(iso3) %>% 
                         slice(1) %>% 
                         select(dc_present = do_downgrade)) %>% 
          mutate(
               dc_present = case_when(
                    is.na(dc_present) ~ FALSE,
                    dc_present == "Yes" ~ TRUE,
                    dc_present == "NO" ~ FALSE,
                    TRUE ~ FALSE),
               vac_coverage = replace_na(vac_coverage, 0)
          )
     

} else {
     x <- x %>% 
          mutate(dc_present = NA,
                 vac_coverage = NA_real_)
}
...
* if decoupling is present & vaccinations/100 persons > 30 & class >= **H** -> **M**
...
make_classif <- function(incid, r, r_lower, r_upper, accel,
  ...
  if (use_dc) {
       if (dc_present & vac_coverage >= 0.3 & out > 2) {
            out <- 2
       }
...
x <- x %>%
  ...
  mutate(
    classif = make_classif(incid = perc_peak,
                           ...
                           accel = net_increases, use_dc = params$use_decoupling, 
                           dc_present = dc_present, vac_coverage = vac_coverage
                           ),
    classif = factor(classif, levels =  c("Minimal", "Low", "Medium", "High", "Very High"))
  ) %>% 
     select(-dc_present, -vac_coverage)
...
lev <- levels(voi$variant)
n_variants <- length(lev) - 1
set.seed(1)
#col_voi <- sample(incidence2::muted(n_variants))
col_voi <- pal_funky(n_variants)
val <- c(col_voi, "grey")
names(val) <- lev
scale_variants_color <- scale_color_manual("", values = val)
scale_variants_fill <- scale_fill_manual("", values = val)


ggplot(voi, aes(x = date)) +
  geom_line(aes(y = mid, color = variant), size = 1) +
  facet_wrap(~ report_country) +
  geom_ribbon(aes(ymin = lower, ymax = upper, fill = variant),
              alpha = 0.3, color = "transparent") +
  theme_bw() +
  scale_variants_color +
  scale_variants_fill +
  scale_x_date(date_labels = "%d %b") +
  labs(title = "Variant distributions over time",
       x = "",
       y = "Variant frequency (% sequenced cases)") +
  theme(legend.position = "bottom",
...
rio::export(out_final, file = "dynamics_synthesis.xlsx", overwrite = TRUE)
...
writeLines(notes, "excluded_countries.txt")
