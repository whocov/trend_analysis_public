...
out_final <- out_full %>%
  ...
  select(
    ...
    growth_rate_upper = r_upper
     ...
     mutate(relative_incidence = scales::percent(round(relative_incidence, digits = 1)))
...
elr_extra_to_keep <- all_elr_extras %>%
  ...
  mutate(
    tpr_recent = scales::percent(round(tpr_recent, digits = 1)),
    tpr_before = scales::percent(round(tpr_before, digits = 1)),
    )
