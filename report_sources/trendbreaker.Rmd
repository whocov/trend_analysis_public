...
analysed_countries <- to_keep <- cases_i %>%
  ...
  filter(total_cases > 10 | total_tests > 10) %>%
  pull(report_country)

notes <- NULL
removed_countries <- setdiff(cases_i$report_country, to_keep)
if (length(removed_countries)) {
  msg <- paste0("The following countries reported < 10 cases/tests over the last 7 weeks ",
...
countries_missing_asmodee <- setdiff(analysed_countries, names(cases_res))
if (countries_missing_asmodee) {
...
```{r eval = (countries_missing_asmodee) > 0), echo = FALSE, results = "asis"}
...
# keep track of countries for which r could not be estimated 
countries_missing_r <- setdiff(cases_i$report_country, r_df$report_country)
...
png(fig_path, width = 25, height = 20, units = "cm", res = 150)
