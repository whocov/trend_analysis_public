...
## # code used to troubleshoot specific sets of countries
## cases_i %>%
##   select(-deaths, -tests) %>% 
##   keep_last(3 * 7) %>%
##   fit_curve() %>% 
##   filter(report_country %in% "Philippines") %>%
##   plot() +
##   scale_x_date()
...
df_peaks <- dat %>%
  ...
  filter(incid_peak > 0) # to avoid dividing by zero
...
if (length(countries_missing_peak)) {
  msg <- paste0("Past incidence peaks could not be found for the following countries:\n",
