...
Estimating TPR is not entirely trivial as some countries report testing *daily*,
some *weekly*, some not at all. The variable `dat$positive_tests` provides a
'naive' estimate of TPR, dividing daily cases by daily tests. Our strategy for
handling TPR is:

* for weekly estimates, re-calculate it from weekly counts of cases and tests
  (rather than averages of daily values, which would grossly under-estimate TPR
  in countries reporting tests weekly)
