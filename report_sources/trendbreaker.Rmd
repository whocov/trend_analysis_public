...
df_r  <- future.apply::future_lapply(
  ...
  cases_i %>%
    ...
    growth_rate() %>%
    mutate(last_day = d) %>% 
    select(report_country, iso3, r, r_lower, r_upper, last_day),
  future.seed = TRUE) %>%
  bind_rows() %>%
...
The variable `dat$positive_tests` provides a 'naive' estimate of TPR, dividing
daily cases by daily tests. However, estimating TPR accurately is not entirely
trivial as there are several situations to cover:

1. countries report testing *daily* with some lags resulting in varying numbers
  trailing 0s in the most recent days
2. countries reporting testing *weekly*, with 0s on all other days
3. countries not reporting testing on a regular basis

For situation 1, we could use the average of the naive estimates, or
re-calculate the ratio excluding the days reporting no tests. However, this
strategy will not work for situation 2, as it would massively under-estimate the
TPR (all weekly cases should be counted, but only one day would
be). Unfortunately, there is no definitive way to disentangle situations 1 from
2: we do not know when days with 0 test reported can be considered as missing
data.

Our strategy for handling TPR is:

* treat 0s as missing data
* for weekly estimates, only estimate TPR when tests have been reported for at
  least 4 days during the last 3 weeks;
* for estimates of change in TPR, fit a linear regression to the 'naive' daily
  TPR over the last 3 weeks, discarding any non-finite values (resulting from
  `0` test reported); only models with at least 14 data points will be used
  
  

## Rate over the last week

We calculate the mean TPR by country over the last week.

```{r }

## # temporary code to check how many TPR can be estimated using different time
## # windows
## days <- 7:30
## n_countries <- lapply(days, function(i)
##   dat %>%
##     filter(date >= (data_stamp - i)) %>%
##     group_by(report_country) %>%
##     summarise(n_non_zero = sum(tests > 0, na.rm = TRUE)) %>% 
##     filter(n_non_zero >= 4) %>%
##     pull(report_country) %>%
##     length()) %>%
##   unlist()
...
# keep only countries reporting tests at least 4 days in the last 3 weeks
to_keep <- dat %>%
  filter(date >= (data_stamp - 21)) %>%
  group_by(report_country) %>%
  summarise(n_non_zero = sum(tests > 0, na.rm = TRUE)) %>% 
  filter(n_non_zero >= 2) %>%
  pull(report_country)
...
df_tpr <- dat %>%
  group_by(report_country) %>%
  filter(date >= (data_stamp - 21),
         report_country %in% to_keep,
         tests > 0) %>%
  summarise(tpr = sum(cases, na.rm = TRUE) / sum(tests, na.rm = TRUE)) %>%
  mutate(tpr = if_else(is.finite(tpr), tpr, NA_real_),
         tpr = if_else(tpr > 1, NA_real_, tpr))
...
To avoid tons or errors due to data quality issue, we only attempt to estimate
changes in TPR for countries for which a TPR could be estimated for the last
week. We also filter out countries with less than 14 TPR estimates over the last
21 days.
...
df_r_tpr <- dat %>%
  filter(report_country %in% to_keep,
         date >= (data_stamp - 21)) %>%
  select(report_country, date, positive_tests) %>%
  filter(is.finite(positive_tests)) %>%
  group_by(report_country) %>%
  filter(n() > 14) %>% 
  nest(data = c(date, positive_tests)) %>%
  mutate(model = map(data, ~ lm(positive_tests ~ date, data = .)),
         r_tpr = map_dbl(model, ~ coef(.)[2]),
         lower_r_tpr = map_dbl(model, ~ confint(.)[2, 1]), 
         upper_r_tpr = map_dbl(model, ~ confint(.)[2, 2]),
         change_tpr = case_when(lower_r_tpr > 0 ~ "increasing",
                                upper_r_tpr < 0 ~ "decreasing",
                                TRUE ~ "constant"),
         change_tpr = factor(change_tpr,
                             levels = c("increasing", "constant", "decreasing")))

```



## Adding TPR info to outputs

We add the information above to `df_dynamics`:
...
df_tpr_info <- df_tpr %>%
  ...
  select(report_country,
         tpr,
         r_tpr, lower_r_tpr, upper_r_tpr,
         change_tpr)

df_dynamics <- df_dynamics %>%
  left_join(df_tpr_info, by = "report_country")
