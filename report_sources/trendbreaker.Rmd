...
1. countries report testing *daily* with some lags resulting in varying numbers
  trailing zeros or NAs in the most recent days
2. countries reporting testing *weekly*, with zeros or NAs on all other days
...
* filter out countries having reported tests for less than 2 days in the last
  week; this will de facto exclude countries reporting tests weekly
* for weekly estimates, re-calculate TPR from weekly counts of cases and tests,
  excluding days where no tests were reported
* for estimates of change in TPR, use the 'naive' daily TPR and discard any
  non-finite values (as days of non-reporting results in either `NA` or `0`
  tests); we'll need to subset models with sufficient degrees of freedom to
...
# keep only countries reporting tests at least 2 days in the last week
to_keep <- dat %>%
  filter(date >= (data_stamp - 7)) %>%
  group_by(report_country) %>%
