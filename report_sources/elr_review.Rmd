...
make_classif <- function(incid, r, r_lower, r_upper, accel,
  ...
  c("No concern", "Low", "Medium", "High", "Very High")[out + 1]
}
...
x <- x %>%
  ...
  mutate(
    classif = make_classif(incid = perc_peak,
                           ...
                           accel = net_increases),
    classif = factor(classif, levels =  c("No concern", "Low", "Medium", "High", "Very High"))
  )
...
out %>%
  mutate(
    ...
    tpr = if_else(is.na(tpr), "", paste(round(100 * tpr), "%"))) %>% 
  filter(classif != "No concern") %>%
