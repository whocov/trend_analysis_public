...
x_all <- x_all %>%
  left_join(x %>% select(report_country, classif))
...
out <- x %>%
  ...
  select(report_country, classif,
         r, r_lower, r_upper,
         perc_peak, net_increases,
         tpr, r_tpr, change_tpr) %>%
  mutate(r = round(r, 3),
...
These pinplots shows all countries classified as 'Mid' and above.
...
plot_tadpole_peaks <- x_all %>%
  ...
  geom_text_repel(
    data = filter(x, classif %in% c("Mid", "High", "Very High")),
    aes(label = report_country), max.overlaps = 100,
                  max.time = 2, max.iter = 1e5) +
  scale_y_continuous(labels = scales::percent) +
  scale_signif +
...
We provide ASMODEE plots for countries classified as "Mid" and above.
...
We plot VoC / VoI dynamics over time for the countries classified as 'Mid' and
...
## ## simplify lineage voi/voc vs the rest
## voi <- voi %>%
...
col_voi <- adegenet::funky(n_variants)
val <- c(col_voi, "grey")
names(val) <- lev
...
rio::export(out_full, file = "dynamics_synthesis.xlsx", overwrite = TRUE)
